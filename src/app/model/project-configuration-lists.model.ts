import { ProjectConfigurationListsInterface, KeyValueInterface } from 'src/app/interfaces/interfaces';
...
export class ProjectConfigurationLists {
	...
	fromInterface(pcl: ProjectConfigurationListsInterface): ProjectConfigurationLists {
		...
		this.extra = pcl.extra.map((item) => { return new KeyValue().fromInterface(item); });
		this.dir = pcl.dir.map((item) => { return new KeyValue().fromInterface(item); });
	...
	toInterface(): ProjectConfigurationListsInterface {
		return {
			...
			extra: this.extra.map((item) => { return item.toInterface(); }),
			dir: this.dir.map((item) => { return item.toInterface(); })
		};
	}
}